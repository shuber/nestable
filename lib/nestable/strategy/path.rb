module Nestable # :nodoc:
  module Strategy # :nodoc:
    ...
    #
    #   +------+---------+--------+------------------------+--------------------+
    #   |  id  |  name   |  path  |  parent_id [optional]  |  level [optional]  |
    #   +------+---------+--------+------------------------+--------------------+
    #   |  1   |  food   |        |           NULL         |         0          |
    #   |  2   |  fruit  |  1/    |            1           |         1          |
    #   |  3   |  sour   |  1/2/  |            2           |         2          |
    ...
    module Path
      def self.included(base) # :nodoc:
        base.class_eval do
          nestable_options.reverse_merge!(:level_column => :level, :path_column => :path, :segment_column => :id, :segment_delimiter => '/')
          # nestable_options[:order] = ["#{table_name}.#{options[:path_column]} || #{table_name}.#{options[:segment_column]} || '#{options[:segment_delimiter]}' || (#{table_name}.#{options[:level_column]}/100000000000)", Array(options[:order])].flatten.compact.join(', ')
      ...
      def descendants # :nodoc:
        self_and_descendants.where("#{self.class.table_name}.#{self.class.primary_key} != ?", id)
      end
      ...
      def self_and_descendants # :nodoc:
        nestable_scope.where("#{self.class.table_name}.#{self.class.nestable_options[:path_column]} LIKE ? OR #{self.class.table_name}.#{self.class.primary_key} = ?",
